...
export function buildWorkspaceDesignerRenderSystem(rubric: string, jsonShape: string): string {
  return [
    ...
    "- Every recommendations.* array must contain at least two distinct, actionable strings grounded in this render and Cisco/Webex workspace guidance.",
    "- Avoid repeating one generic phrase across categories; cite Resource URLs from the rubric where appropriate.",
    "",
    rubric,
    "",
    jsonShape,
